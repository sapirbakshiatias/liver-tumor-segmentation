"""
Spatial feature heatmaps overlaid on CT slices.

For each selected feature, a sliding window computes the local feature value
at every position inside the liver mask. The result is overlaid as a heatmap
on the CT image, showing WHERE in the liver the feature discriminates.

High values (bright red) in cancerous livers reveal the spatial pattern
of the discriminative feature — e.g., sagittal_glcm_dissimilarity shows
the tumor's patchy texture as bright regions.
"""
import os
import numpy as np
...
WINDOW   = 16   # sliding window size in pixels
STRIDE   = 4    # step between windows
COLOR_CANCER  = "#C62828"
COLOR_HEALTHY = "#1B5E20"


def _load_best_slice(series_stem):
    """Load the axial slice with the most liver pixels."""
...
def _patch_value(pix, feature_name):
    """Compute a single feature value for a patch of liver pixels."""
...
def _sliding_map(ct_sl, mask_sl, feature_name):
    """Compute feature value in a sliding window over the liver region."""
    h, w = ct_sl.shape
    ...
    for y0 in range(0, h - WINDOW + 1, STRIDE):
        for x0 in range(0, w - WINDOW + 1, STRIDE):
            ...
            val = _patch_value(pix, feature_name)
...
def plot_spatial_heatmaps(df, feat_names):
    """Generate one PNG per feature showing the feature map on all series."""
    ...
    for feat in feat_names:
        ...
        for idx, (stem, label) in enumerate(zip(series_list, labels)):
            ax = axes[idx]
            ct_sl, msk_sl = _load_best_slice(stem)
            if ct_sl is None:
                ax.axis("off")
                continue

            # CT background in grayscale
            ...
            # Feature overlay inside liver mask only
